...
//     cout << "The ZMQ_req client is connect...." << endl;
    ...
    thread get_obj_info(detect_trafficcone, argc, argv);
    sleep(1);
    thread send_msg(send_info);
    sleep(1);
    // thread server_pulse(send_pulse);
//    sleep(2);
//    get_rgb_img();
//
    get_obj_info.join();
...
//     int lastTime = 0;
//     zmq::context_t context_port(1);
//     zmq::socket_t server_send(context_port, ZMQ_REQ);
// // //    reqer.setsockopt(ZMQ_RCVTIMEO,2000);
//     server_send.connect(pbREQ_IP_PORT);
//     while (1) {
//         // int now = clock()/CLOCKS_PER_SEC;
//         int now = getTime();
//         if (now - lastTime == 5) {
//         string get_url = "name=detector3;action=get_rtsp_url";
//         zmq::message_t pulse(strlen(get_url.c_str()));
//         memcpy((void *) pulse.data(), get_url.c_str(), strlen(get_url.c_str()));
//         server_send.send(pulse);
...
// void send_pulse_5s(zmq::socket_t &reqer) {
//     int now = clock()/CLOCKS_PER_SEC;
//     if (now - lastTime == 5) {
//     string get_url = "name=detector3;action=get_rtsp_url";
//     zmq::message_t pulse(strlen(get_url.c_str()));
...
void detect_trafficcone(int argc, char **argv) {
    ...
    string name = "name=detector4;action=get_rtsp_url";
    zmq::message_t message(strlen(name.c_str()));
    memcpy((void *) message.data(), name.c_str(), strlen(name.c_str()));
    reqer.send(message);
    zmq::message_t reply;
    ...
    while (ret) {
        ...
        if ((now - lastTime) == 5) {
            string get_url = "name=detector4;action=get_rtsp_url";
            zmq::message_t pulse(strlen(get_url.c_str()));
            memcpy((void *) pulse.data(), get_url.c_str(), strlen(get_url.c_str()));
            reqer.send(pulse);
            // reqer.send(message);
        ...
        // Mat frame = VideoCapture.open(url);
        cap >> frame;
        resize(frame, frame, Size(1280, 720));
        // imshow("rtsp_url", frame);
        // waitKey(10);
        ...
        int traffic_num = 0;
        int frameIndex = 3;
        string name = "name=AI;event=4;para=";
...
//        {"0":"truck"; "1", "car"; "2", "bus"; "3", "van"; "4", "person";}
        ...
        for (auto b : remaining) {
            if (to_string(b.label) == "0") {
                ...
                string objInfo;
                objInfo.clear();
                objInfo = "trafficcone," + to_string(left) + "," + to_string(top) + "," +
                          to_string(width) + "," + to_string(height) + ",";
                objBoxesInfo = objBoxesInfo + objInfo;
                // objBoxesInfo = "vehicle,";
                rectangle(frame, Rect(left, top, width, height), cv::Scalar(255, 0, 0), 2);
            }
        }
...
//        imginfo.first=frame;
        imshow("detector trafficcone", frame);
        waitKey(10);
...
//        for(int i=0;i<signed(sizeof(msg_content.chStr_info));i++) msg_content.chStr_info[i] = 0;
        ...
        cout << "global_send_msg.length: " << global_send_msg.length() << endl;
//        cout << "already prepare the send msg..." << endl;
        ...
        cout << "frTmp size: " << frTmp.size() << endl;
        cout << "msg_content.chStr_info: " << msg_content.chStr_info << endl;
        // cout << "msg_content.size(): " << sizeof(msg_content) << endl;
