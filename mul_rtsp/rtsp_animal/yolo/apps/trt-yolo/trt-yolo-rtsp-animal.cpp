...
//     cout << "The ZMQ_req client is connect...." << endl;
    ...
    thread get_obj_info(detect_highway_animal, argc, argv);
    sleep(1);
...
void detect_highway_animal(int argc, char **argv) {
    ...
    string name = "name=detector5;action=get_rtsp_url";
    zmq::message_t message(strlen(name.c_str()));
    memcpy((void *) message.data(), name.c_str(), strlen(name.c_str()));
    reqer.send(message);
    zmq::message_t reply;
...
//    string state ="";
    ...
    while (ret) {
        ...
        if ((now - lastTime) == 5) {
            string get_url = "name=detector5;action=get_rtsp_url";
        ...
        int animal_num = 0;
        int frameIndex = 4;
        string name = "name=AI;event=5;para=";

//        {"0":"truck"; "1", "car"; "2", "bus"; "3", "van"; "4", "person";}
        // {"0":"cat", "1":"dog", "2": "horse", "3": "sheep", "4": "cow", "5":"elephant", "6":"bear", "7": "zebra", "8":"giraffe"}
        ...
        for (auto b : remaining) {
            ...
            if (int(b.label) < 10) {
                ...
                string objInfo;
                objInfo.clear();
                objInfo = "animal," + to_string(left) + "," + to_string(top) + "," +
...
//            else if(state == "start") state="keep";
            ...
            no_animal_frame++;
            if (no_animal_frame > 50) state = "end";
            if (state == "start") state = "keep";
        }

        string reTime = get_Ntp();
        if (animal_num > 0) objBoxesInfo.pop_back();

        string zmq_send_msg = name + to_string(frameIndex) + ","
                              + to_string(detectIndex) + ","
                              + state + "," + reTime + ",["
                              + to_string(animal_num) + ","
                              + objBoxesInfo + "];";
...
//        for(int i=0;i<signed(sizeof(msg_content.chStr_info));i++) msg_content.chStr_info[i] = 0;
        ...
        // cout << "global_send_msg.length: " << global_send_msg.lesngth() << endl;
