...
void detect_highway_person(int argc, char **argv) {
    ...
    string name = "name=detector2;action=get_rtsp_url";
    zmq::message_t message(strlen(name.c_str()));
    memcpy((void *) message.data(), name.c_str(), strlen(name.c_str()));
    reqer.send(message);
    zmq::message_t reply;
...
//    string state ="";
    ...
    while (ret) {
        ...
        cap >> frame;
        // resize(frame, resized_image, Size(1280, 720));
        ...
        int person_num = 0;
        int frameIndex = 1;
        string name = "name=AI;event=2;para=";
...
//        {"0":"truck"; "1", "car"; "2", "bus"; "3", "van"; "4", "person";}
        ...
        for (auto b : remaining) {
            if (to_string(b.label) == "1") {
                ...
                string objInfo;
                objInfo.clear();
                objInfo = "car," + to_string(left) + "," + to_string(top) + "," +
